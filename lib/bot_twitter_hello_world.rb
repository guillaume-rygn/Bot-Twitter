...
def pick_rand_handle(handle)
  rand_handle =[]
  i = 0

  while i <=5
    rand_handle << handle[rand(handle.length)]
    i +=1
  end
...
def send_tweet(rand_handle, client)
  comment =["incroyable article", "hate de voir tes prochaines publications", "Si la méchanceté suffisait pour faire fortune, il y a beaucoup de journalistes qui seraient célèbres.","cet article a changé ma perspective","jolie travail"]

  for a in 0..4 do
    #client.update("#{comment[a]} #{rand_handle[a]}\n #bonjour_monde @the_hacking_pro")
  end
end

def like_tweet (client) #like 25 tweet recent contenant le #bonjour_monde
  tweets = client.search("#bonjour_monde", result_type:"recent").take(25)
  #client.favorite(tweets)
end

def follow_tweet (client) #follow 20 personnes qui ont tweet recemment contenant le #bonjour_monde
...
def like_and_follow_stream(client, client_stream)
  ...
  client_stream.filter(track: "#bonjour_monde") do |tweet|
    if tweet.is_a(Twitter::Tweet)
        ...
        puts "Je viens de follow : #{tweet.user.id}\n\n"
    end
  end
end
...
def perform 
  handle = handle_array
  rand_handle = pick_rand_handle(handle)
  client = login_twitter
  send_tweet(rand_handle, client)
  like_tweet(client)
  follow_tweet(client)

  client_stream = stream_config
  like_and_follow_stream(client, client_stream)
